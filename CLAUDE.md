...
- `utils/refBadgeContent.ts` — a ref badge's label, icons and tooltip. **The two branch glyphs are a system**: the fork means "exists locally", the cloud means "exists on a remote", so a branch that is both reads as the union (`⑂ main ☁`) and the scheme is legible without hovering. Remote *names* live only in the tooltip; 2+ remotes add a count to the cloud (`⑂ main ☁2`), because a badge should spend row width only on what can't be inferred. Adding a cloud that means anything else breaks the encoding
