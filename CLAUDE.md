...
- **Splitting a graph editor opens another graph.** A webview cannot be duplicated, so VS Code answers "Split Editor Right" with an *empty* editor group and offers no command to intercept. `ExtensionController` therefore recognises the split from its aftermath — `utils/editorSplitFill.ts`: a group that opened empty while the group the user was in had a graph as its active tab — and fills it exactly like a new tab. One rule covers the title-bar button, `Ctrl+\`, split down and split left; it re-reads the group a tick later so a tab *dragged* into a new group keeps it.
...
- `utils/commitMenuAvailability.ts` — which commit actions apply (rebase/reset/revert/drop/cherry-pick/merge/amend/fixup); shared by the commit row menu and the badge "Commit actions" submenu
...
- `utils/refWorktreeSource.ts` — the same shape of answer for "Create worktree…": which ref the badge hands to `git worktree add`, `<remote>/<name>` again for a remote branch, plus the `WorktreeSource` types. It deliberately does **not** ask whether a local branch of that name exists — `git worktree add origin/foo` is fine while a local `foo` sits on another commit, and that divergence is the usual reason to want the worktree. A name that turns out to be taken is git's to refuse in the dialog
- `utils/refBadgeContent.ts` — a ref badge's label, icons and tooltip. **The two branch glyphs are a system**: the fork means "exists locally", the cloud means "exists on a remote", so a branch that is both leads with the union (`⑂☁ main`) and the scheme is legible without hovering. Both glyphs *lead*, which keeps the cloud on the same side of the name in every badge and leaves the trailing slot to the worktree icon alone. Remote *names* live only in the tooltip; 2+ remotes add a count to the cloud (`⑂☁2 main`), because a badge should spend row width only on what can't be inferred. Adding a cloud that means anything else breaks the encoding. It also owns `hiddenSearchTexts` — text a badge can *match* on but does not render (a merged branch's qualified `origin/name`) — so the search matcher, the highlighter and `RefLabel` all read one answer to "what does this badge show, and what does it hide?"; a second `switch` over `DisplayRef` is how a row lights up with nothing boxed
- `utils/searchQuery.ts` / `utils/searchFilter.ts` / `utils/searchHighlight.ts` — **the search query is not one literal string.** It splits on whitespace into terms that are AND-ed, a `"quoted run"` being one literal term, and each rule is evaluated *per term*: in `fix a1b2c3d` only `a1b2c3d` is hash-eligible, because the 4-character hash floor (git's own, so `add`/`beef` can't light up unrelated rows) belongs to the term rather than the query. An **unterminated quote is literal from the quote onward**, so results don't flash between meanings mid-keystroke, and `:` is reserved so field qualifiers (`author:john`) can land later without changing what a literal term means. `parseSearchQuery` allocates, so it is called **once per recompute** in `SearchWidget` and the result is stored in `graphStore.searchTerms` — a fresh array per render would defeat `CommitTableRow`'s memo for every row on screen. Matching and highlighting must be driven by that one term list: `matchIndices` and `searchTerms` are written in a single `set` call so the panel's "N of M" and the rows' boxes can never describe different queries
- `utils/mergeRefs.ts`'s `filterDisplayRefsBySettings` — **the single source of truth for which ref badges a row shows.** The renderer and the search matcher both call it, which is what makes "only a badge you can see can match" true by construction rather than by two copies of one rule agreeing. It *degrades* a merged branch to a local one when remote branches are hidden rather than dropping it, so `main` still matches while `origin/main` stops
...
- `utils/branchCheckout.ts` + `components/BranchCheckoutDialogs.tsx` — the shared menu/double-click checkout workflow. Local badges double-click without pulling; a remote badge with a local counterpart opens the existing Pull/No pull dialog. Dialogs are owned once by `App`, never by a lazy menu or virtualized row. `BranchCheckoutRequest` preserves repository, exact remote (tracking creation only), and request ID through stash recovery; `branchCheckoutFinished` releases the client busy state only for that request. Backend `branchCheckoutHandlers` guards operations and retains the original services across stash/checkout/pull, suppressing stale results after navigation. Never bypass this flow with a badge-local RPC or silently change the local branch's upstream.
- `utils/branchSelection.ts` — `getBranchKey` (bare name vs `remote/name`) + additive select-all-local
- `utils/resolveDefaultRemote.ts` — pick `origin` else first-alpha remote for general defaults. `resolvePublishedBranchRemote` is the stricter amend force-push resolver: use a matching upstream or a sole same-named remote counterpart; never fall back to an unrelated remote or guess an ambiguous destination.
...
- `utils/autosquash.ts` — git's autosquash matching **ported from `sequencer.c`**, used by both rebase dialogs: exact subject (earlier commits only, first wins) → hash prefix (4+ hex digits, git's abbreviation floor) → subject prefix; nested `fixup! squash! X` → `X`, outermost prefix decides the kind. `amend!` is applied as target `reword` + `fixup` (same tree as `fixup -C`, any git version). Don't re-derive these rules in a dialog
...
- `components/dialogStyles.ts` — shared dialog sizing, **the three button variants** (`buttonPrimaryClassName`/`buttonSecondaryClassName`/`buttonDangerClassName`, all composed from one base so shape and disabled behaviour cannot drift) `dialogSectionLabelClassName` for settings-group captions, `dialogMessageTextareaClassName` for commit-message boxes, `dialogOverlayClassName` for the scrim and **the three inline message boxes** (`dialogNoteClassName`/`dialogWarningClassName`/`dialogErrorClassName`, likewise one base); every `Dialog.Content` uses `dialogContentClassName` + `dialogContentStyle`, and every `Dialog.Overlay` the overlay constant. Never hand-write a button's classes — the variants carry `disabled:` handling that inline copies kept getting wrong
- `components/ResponsiveToolbar.tsx` — measures actual button widths and collapses the right group before the left. Reuses the mounted controls in dropdowns so labels, visibility settings, dialogs and context menus stay intact; add actions to the existing groups without adding breakpoints.
...
- `components/whatsNewEntries.tsx` — **release notes are added here and nowhere else.** One entry per version, matched by *exact* `package.json` version; a version with no entry shows no dialog, which is how a release opts out. Content is a `ReactNode`, so an entry can embed live UI (5.10.0 embeds `RefBadgeLegend`) rather than describing it. Its look is governed by the design requirements below. **Never add, change or skip an entry on your own judgement.** The maintainer runs this: before a release they start a What's New pass with you and decide, item by item, what goes in and whether the dialog shows at all. Your job is to ask, not to decide — clarify every time, and a reminder that the pass is due is welcome. Not raising it is also fine; they initiate it themselves. A test therefore cannot require an entry for the shipping version; it instead asserts every entry's version is an exact `N.N.N` string, which is what catches the typo that would otherwise mean no dialog ever.

  **What's New Design requirements — the 5.16.0 entry is the reference implementation; match it.** The dialog advertises a feature, so it must look like a poster, not a wall of text a user reflexively closes:

  - **Hero**: version as a small pill, the headline in large bold type over the gradient, and an `illustration` beside it when the feature can be shown — ideally a small animated miniature of the real UI (`AutosquashIllustration`), static under reduced motion.
  - **Compose from `whatsNewBlocks.tsx`, never paragraphs**: `ContributorThanks` (avatar ribbon) first when a contributor was involved, `WhatsNewSection` captions, `FeatureCard`s in a `FeatureGrid` for parallel options, `StepFlow`/`Step` for a workflow. Keep each card's text to a sentence or two.
  - **Name the git flag** on a card that maps to one (`gitFlag="--fixup"`), so git users can connect the UI to what they already know.
  - **Every menu item or control name is a `UiLabel`** — the orange marker highlight, which wraps rather than overflowing a narrow card — so readers can spot the exact text in the real UI. Spell it exactly as the menu does.
  - **Alternatives are not a sequence**: label them `2A`/`2B` and mark the second `alternative` (draws the "or" joint), rather than numbering choices as consecutive steps.
  - All colors via `themeColors.ts` (`SHOWCASE_*` accents + `tint()`); the layout must still work in a narrow panel (grids collapse to one column).

  Their general leanings — still confirm each release rather than applying these silently:

  - **A contributor was involved → always credit them**, thanking them in the entry. This holds even for a fix-only release; 5.10.1 got a dialog purely to credit a first-time issue reporter.
  - **Changes by the maintainer alone → new features are shown, bug fixes are skipped.** A fix does not go in an entry even when it ships beside a feature in the same release; `CHANGELOG.md` is where fixes are recorded.
  - **A release that is only bug fixes → the dialog may be skipped entirely**, which a version with no entry does by itself
- `hooks/useDialogTelemetry.ts` — one confirmed/cancelled outcome per dialog open cycle
- `hooks/useCommitHookWait.ts` + `components/CommitHookWait.tsx` — the running → "waiting on hooks" phase, its notice and the Cancel / "Cancel wait" button for any dialog that writes a commit (amend, fixup). The button sends the one `cancelCommitWait` RPC itself; the backend's matching half is one `WebviewRuntime.activeCommitController`, held by `commitHandlers`' shared cancellable runner
- `hooks/useCopyFeedback.ts` — `copyToClipboard` + short "copied" flash, used by every copy button

### Telemetry Requirements

- **Every new or enhanced feature must include a telemetry review and applicable instrumentation.** Track user-initiated Git operations, branch-badge double-click checkout, and the approved UI surfaces: context-menu items, toolbar buttons, dialog outcomes, panel toggles, and column visibility changes. Do not track chatty/background interactions such as loading, scrolling, hovering, typing, or auto-refresh.
