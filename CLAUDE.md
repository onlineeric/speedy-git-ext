...
- `components/dialogStyles.ts` — shared dialog sizing, **the three button variants** (`buttonPrimaryClassName`/`buttonSecondaryClassName`/`buttonDangerClassName`, all composed from one base so shape and disabled behaviour cannot drift) `dialogSectionLabelClassName` for settings-group captions, `dialogOverlayClassName` for the scrim and **the three inline message boxes** (`dialogNoteClassName`/`dialogWarningClassName`/`dialogErrorClassName`, likewise one base); every `Dialog.Content` uses `dialogContentClassName` + `dialogContentStyle`, and every `Dialog.Overlay` the overlay constant. Never hand-write a button's classes — the variants carry `disabled:` handling that inline copies kept getting wrong
...
- `hooks/useCommitHookWait.ts` + `components/CommitHookWait.tsx` — the running → "waiting on hooks" phase, its notice and the Cancel / "Cancel wait" button for any dialog that writes a commit (amend, fixup). The backend's matching half is one `WebviewRuntime.activeCommitController`
