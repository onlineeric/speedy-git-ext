...
- **An optional force push after amending.** Amending rewrites the commit, so a commit that already exists on a remote will need a force push — the dialog says so, and offers "Force Push after amended" as a checkbox, which the preview spells out as `--force-with-lease`. The checkbox appears only when the commit is actually published and there is a branch to push, so its absence is itself the signal that this commit is yours alone. The push runs as a separate step after the amend has succeeded, and the two report separately: a rejected push always reads as "amended locally, force push rejected", never as a failed amend. A lease rejection — which git reports as `stale info` — is translated into what actually happened: the remote moved since your last fetch, so fetch and look before pushing again.
- **Notes for the two consequences you can see afterwards.** A signed commit gets a note saying the signature is replaced — re-signed with your own key if you sign, unsigned otherwise — because the Signature column is where you will notice it. A published commit gets the rewrite warning above. Neither blocks anything: Speedy Git does not veto what git itself would accept.
