...
### Added
- **Toolbar buttons stay accessible in narrow views.** The right-hand button group collapses into a three-dot dropdown first, followed by the left-hand group when space gets tighter. Both groups return as space becomes available. Dropdown buttons keep your label and visibility settings and their right-click menus; separators appear only in the toolbar. The layout adjusts to the actual button widths, including when labels or buttons are shown or hidden.
- **Multiple graph tabs in one window.** A new "Open New Graph Tab" button sits beside Go to HEAD in every graph's toolbar, with a matching **Speedy Git: Open New Graph Tab** command in the Command Palette. Each click opens another graph immediately — no picker, no confirmation — on the originating graph's repository, in the same editor group. The same repository may be open in several tabs. Each tab keeps its own repository and submodule selection, filters, search, selected commit, compare slots, column layout and scroll position, so one view can follow a release branch while another searches development history, or a parent repository can sit beside its submodule. Arrange, split and close them with VS Code's own editor controls; closing the last one is fine, and the next ordinary open creates a graph normally. Tabs are session-only: reloading the window does not restore them.
- **Opening Speedy Git returns to the graph you were last in.** The Show Speedy Git command, `Ctrl/Cmd+Shift+G` and the status bar item now reveal the most recently active graph, with its investigation and editor group untouched, and create one only when none is open. **Open in Speedy Git** in the Source Control view is repository-aware: it reveals a graph already showing that repository — including one currently displaying a submodule of it — and otherwise opens a new one. Neither ever retargets an open graph to a different repository.
- **A notice when another view is mid-operation.** Graphs showing the same working tree say so while one of them runs a Git operation. It is a notice only: no button, menu item or dialog is disabled by another view's work, because the only way to cancel lives in the view that started it. Genuinely conflicting operations are still reported by Git itself.
- **Reset, rebase, force-push, delete branch and drop commit now refuse to act on a ref that moved.** These are the five actions Git performs without complaint against a ref that changed under an open dialog — `reset --hard`, `push --force` and `branch -D` all succeed regardless. The target is now re-read immediately before running, and if it moved the action does nothing and says so, so a plan built from the commits you saw is never replayed against a different tip. Reopen the dialog to act on the refreshed state.
...
### Added
- **Search now matches branch names, tag names and author emails, not just messages, author names and hashes.** In a large repository you usually remember *who* wrote a commit or *which* branch or tag it belongs to rather than the exact words of its message — but ref badges were invisible to search, so the one thing on the row you were reading was the one thing you could not search for. A term now matches a local branch name, a tag name, a remote branch name and an author's email address as well, all as substrings, exactly like the message already did: `feature` finds `feature/new-ui`, `v1.2` finds `v1.2.0`. A remote branch matches under both its bare and its qualified name, so `main` and `origin/main` both find `origin/main`. Commit hashes keep their existing rule — a prefix, four characters or more, mirroring how git itself resolves an abbreviation and stopping short hex-ish words like `add`, `face` and `beef` from lighting up unrelated commits.
- **The query is now several terms, not one literal string.** Typing `john fix` used to search for the literal text `john fix` and find nothing, even though what you meant was obvious: a commit by John about a fix. The query is now split on whitespace into terms, and a commit matches when *every* term matches it somewhere — different terms may land on different fields, and two terms may land on the same one, so `fix login` still finds "fix the login bug". A `"quoted phrase"` is one literal term, spaces included. That is the whole of the syntax: no regex, no case toggle, no whole-word toggle, no fuzzy matching, and nothing new in the panel to learn it from. Each rule applies per term rather than to the whole query, so in `fix a1b2c3d` only `a1b2c3d` is eligible to match a hash. While you are typing an unterminated quote it behaves as a literal, so results do not flip between meanings mid-keystroke.
- **The matched text is now boxed inside the row, not just the row highlighted.** With eight fields able to match, a highlighted row no longer says *why* it matched. Matches are now boxed where they land — inside the message, the author name, the abbreviated hash and the ref badge itself — and with several terms, every term's match is boxed in every cell it hits. A merged branch badge shows only its local name, so when a term matches it through `origin/main`, which the badge never displays, the whole badge is ringed instead: still a visible reason, since a match you cannot see is worse than no match.

### Changed
- **Only refs you can actually see can match.** With **Show tags** or **Show remote branches** turned off in View settings, those names do not match. Those settings say "I do not care about these refs", and a row lighting up with nothing on screen explaining why is worse than no match at all. Hiding a *column* is different — that is a choice about screen width, not about what matters — so a hidden Author, Hash or Message column changes nothing about what matches. The one deliberate asymmetry is the author email, which matches although only the name is displayed: the email is what you actually remember and type, and it is already visible on hover.
- **The current match now survives a batch load, a filter change or another keystroke.** Every recompute used to snap you back to the first result, so scrolling further into history while parked on match 40 of 60 lost your place. If the row you are on still matches, you stay on it, the way a text editor's find does.
- **"No results" now says what was searched.** Search only ever covers the commits already loaded into the graph — it does not fetch further batches and does not fall back to a repository-wide git query — so a bare "No results" read as a bug to anyone who knew the commit existed. It now reads "No results in N loaded commits", which says that scrolling further will bring more into range. The tip beside it now names what is searchable and how the words combine — that every word must match and that `"quotes"` make an exact phrase — rather than pointing at the Filter panel, since neither the widened scope nor the term syntax is discoverable otherwise.
- **The uncommitted changes row is never a match.** Its subject is generated text like "3 staged, 2 unstaged", so a search for `staged` surfaced it oddly — and it sits at the top of the graph, where it never needs finding.

### Credits
- Raised by [@nelson870708](https://github.com/nelson870708) in [#191](https://github.com/onlineeric/speedy-git-ext/issues/191), asking that search reach beyond commit messages to branches, tags and authors. Thank you!

## [5.13.0] - 2026-08-29

### Added
- **The Create Worktree dialog now offers a nested folder as well as a flattened one.** Branch names use `/` as a namespace separator — `feat/branch1`, `exp/20260826-test` — and the suggested worktree folder always flattened that separator to `-`, so the hierarchy you already organise your branches by was thrown away and a busy repo ended up with a flat list of long folder names. Plain `git worktree add <base>/exp/20260826-test` works and creates the intermediate folders itself, so the flattening was the extension's own invention. But it is not simply wrong either — other tools flatten deliberately so the folder tree is not full of single-child directories, and the mirror-image complaint has been filed against editors that nest. It is a preference split rather than a bug, so both are offered rather than one being swapped for the other. For a branch name containing `/`, the "Worktree folder" field becomes two rows — **Nested path** and **Flatten path** — each showing its own complete, concrete path. The selected row's box is editable exactly as the single box was; the other is readable but disabled, so the two can be compared before choosing. The command preview and the Create button always follow the selected row. A branch name with no `/` is untouched: one label, one box, exactly as before, and the rows appear and disappear live as you type a `/` into a new branch name.
- **A one-click way to make your choice the default.** A small link under the two rows — "Use Nested path by default" / "Use Flatten path by default" — writes `speedyGit.worktree.folderNameStyle` and then disappears, because the selection now matches the default. It shows only when clicking it would actually change something, and it saves the *style* only: whatever you typed into the path box is a one-off for this worktree and is never saved. It writes to your User settings normally, but to Workspace settings if the setting is already defined there — otherwise the workspace value would keep winning, the link would not disappear, and the click would look broken.
- **Each option's path is collision-checked on its own.** If `<base>/feat-branch1` already exists but `<base>/feat/branch1` does not, the two boxes show `feat/branch1` and `feat-branch1-2`. That asymmetry is deliberate: only one of them will be created, and each box should show a path that is genuinely free. A suffix always lands on the last segment, never on a parent folder.
- **Empty parent folders are cleaned up when a worktree is removed.** Removing `<base>/exp/branch1` deletes only `branch1`, leaving `<base>/exp/` behind forever — over a year of short-lived branches that accumulates. Removing a worktree through Speedy Git now also deletes the folders that removal just emptied, walking upward and stopping before the configured base path, which is never touched. It applies only to worktrees inside that base path — one you placed somewhere custom is left entirely alone — and never deletes a folder containing anything at all, including hidden files. Pruning stale worktrees sweeps the base path the same way, so parents orphaned by a folder you deleted outside VS Code are cleaned up too. All of it is silent: these are empty folders the extension created, and clearing them up is housekeeping rather than an action worth reporting.

### Changed
- **Worktree folders are now labelled by their path below the base path.** Nesting makes the last path segment ambiguous — `exp/branch1` and `feat/branch1` would both have read as `branch1` in the Worktrees menu and the detached-worktree badge. A worktree inside the configured base path now shows the part below it, so the two are told apart; one outside the base path keeps its last segment, since there is nothing meaningful to show it relative to. Full paths remain in the tooltips.
- **Switching between the two folder options warns before discarding an edit.** At most one path can hold manual edits at a time — the selected one — so leaving an option restores it to its computed default and there is never a hidden edit sitting in the row you are not looking at. If the box you are leaving has actually been changed, a small confirmation asks first, with Discard focused so Enter proceeds; cancelling leaves both the selection and your text exactly as they were, and the radio does not visibly move and move back. Typing something and deleting it again counts as no edit, so it does not prompt. Changing the branch name or branch mode re-derives both paths and silently discards an edit — the branch name is the source of truth for the suggestion, and confirming on every keystroke would be unusable.

### Fixed
- **"Create worktree…" is no longer missing from a remote branch badge whose local counterpart has moved on.** The item was shown only when no local branch of that name existed anywhere in the repository — but a remote badge appears on its own row precisely when the two have *diverged*, since a local and remote branch sitting on the same commit are drawn as one merged badge. So the item was hidden in exactly the case where it was most useful: `origin/feature` has commits your local `feature` does not, and you want a worktree to look at them. It now appears on every remote branch badge, basing the worktree on `origin/feature` — the fully qualified name, so it cannot silently resolve to the local branch of the same name. If the new branch name it suggests turns out to be taken, git says so and the dialog stays open for you to change it, rather than the extension deciding in advance that you could not have meant it.

### Credits
- Raised by [@nelson870708](https://github.com/nelson870708) in [#189](https://github.com/onlineeric/speedy-git-ext/issues/189), asking that the branch directory structure be preserved when creating worktrees. Thank you!

## [5.12.0] - 2026-08-25

### Added
- **Amend Last Commit.** Speedy Git could stage, unstage and discard, but it could not create or modify a commit — the highest-frequency half of that gap being the typo you notice in the message you just wrote, or the one file you forgot to include. Right-clicking the commit git currently has checked out now offers "Amend Last Commit...", which opens a dialog prefilled with that commit's complete existing message, body and trailers included. Rewriting the tip is the safe half of rewriting history: it is a single step with no todo list, no replay of later commits, and therefore no possibility of conflicts. Rewording an *older* commit is a different animal and remains the interactive rebase's job. The item appears on the checked-out row — including in detached HEAD, where git amends perfectly well — and on every ref badge sitting on that row. The badge you open changes nothing about what happens — every badge on a row sits on the same commit, and amend rewrites that commit — so it behaves like the other current-branch actions already in those menus: Revert, Drop Commit and Reset. What it does *not* change is which branch follows the rewrite: `git commit` takes no branch argument, it moves whatever HEAD points at, so if a second branch sits on the same tip that branch stays on the original commit while the checked-out one moves. The dialog names the branch that will move, rather than leaving that to be inferred. It never appears on a stash entry, is disabled rather than hidden while a rebase, merge, revert or cherry-pick is in progress, and is refused by the backend as well for one started in a terminal a moment ago.
- **Staged changes are only folded in if you ask.** Plain `git commit --amend` absorbs whatever is in the index, which is the classic amend footgun: files staged for the *next* commit are silently swallowed into a rewrite you cannot undo from the UI. The dialog instead shows a checkbox naming how many files are staged, off by default. Left off, the message alone changes and anything staged stays staged; the command preview shows the real difference between the two forms as you toggle it.
- **An optional force push after amending.** Amending rewrites the commit, so a commit that already exists on a remote will need a force push — the dialog says so, and offers "Force Push after amended" as a checkbox, which the preview spells out as `--force-with-lease`. The checkbox appears only when the commit is actually published *and* the checked-out branch has a remote counterpart of its own, so its absence is itself the signal that this commit is yours alone. Those are two different questions and they come apart: when an unpublished branch happens to share a tip with a published one, the commit is on a remote while the branch is not — offering a force push there would publish that branch for the first time, under a label that says force push, on a branch whose absence of a remote is exactly what made it private. The push runs as a separate step after the amend has succeeded, and the two report separately: a rejected push always reads as "amended locally, force push rejected", never as a failed amend. A lease rejection — which git reports as `stale info` — is translated into what actually happened: the remote moved since your last fetch, so fetch and look before pushing again.
- **Notes for the two consequences you can see afterwards.** A signed commit gets a note saying the signature is replaced — re-signed with your own key if you sign, unsigned otherwise — because the Signature column is where you will notice it. A published commit gets the rewrite warning above, under the same pair of conditions as the checkbox — where the commit is published only through some other branch, this branch's amend rewrites nothing that is out there, so there would be nothing to force push. Neither blocks anything: Speedy Git does not veto what git itself would accept.

### Changed
- **A slow commit hook now reads as your tooling rather than a frozen extension.** Committing runs `pre-commit` and `commit-msg` hooks — git runs them even for a message-only amend, since it does not skip them just because the tree is unchanged — and in a husky/lint-staged repo that is tens of seconds during which nothing is visible. Amend gets a 60-second ceiling rather than the usual 30, and after about three seconds the dialog says it is waiting on hooks and offers to stop waiting. Both cancelling and hitting the ceiling end *our* wait, not the hook process, which may still be modifying files afterwards — so both say so. Neither guesses the outcome: hooks run before the commit object is written, so almost always nothing was created, but killing git in the window between the object being written and the process exiting leaves the amend done. HEAD is re-read afterwards and the result reports whichever of the two actually happened.
- **The dialog stays open on every failure, with your message intact.** A `commit-msg` hook that rejects the message at second 40 must not take the message with it. It closes on success and when you cancel it, and never on an error. That includes HEAD moving out from under it: `git commit --amend` has no target — it rewrites whatever HEAD is when it runs — while the dialog shows one specific commit and prefills that commit's message. Since commit creation is delegated to VS Code's SCM view, committing from another panel of the same window is an ordinary thing to do, and without a check the old commit's message would be silently written onto a different commit, destroying that one's message and leaving the intended one untouched. The commit's identity is captured when the dialog opens and verified immediately before the amend runs; on a mismatch nothing is amended and nothing is re-targeted.

### Fixed
- **Interactive rebase no longer discards a commit's body when you reword or squash it.** The reword box was prefilled with the commit's subject only, and whatever sits in that box becomes the commit's entire new message — so rewording a multi-paragraph commit silently threw away its body and any trailers, including `Co-authored-by:` lines, even if you changed nothing but a typo in the first line. Squashing had the same problem from the other direction: the combined message was built by joining subjects, so every body in the group was lost. Both now read and write the complete message. Nothing in the extension read a commit's full raw message before this release, which is why the bug existed at all.
...
- **Uncommitted submodule changes could not be opened at all.** For everything else the working-tree side of a diff is the file on disk, but a checked-out submodule is a directory, and the diff editor has no way to open one — so this case failed rather than merely showing blank. It now resolves the commit the submodule is currently checked out at and shows that as the pointer, marked `-dirty` exactly as `git diff` marks it when the submodule has uncommitted work of its own — without which a submodule listed as changed purely because its checkout is dirty would show the same commit on both sides, and look blank all over again. A submodule that has never been initialized, or one in the middle of a merge conflict, has no commit to point at, and that side is simply left empty rather than reporting a commit from the wrong repository or an all-zero hash that names nothing.
...
### Added
- **A short "What's new" dialog the first time you open the graph after an update.** Release notes have only ever lived in the changelog on GitHub, which is not where anyone is looking on the morning VS Code quietly auto-updates an extension — so a change like this release's new branch badges would simply turn up one day with nothing to explain it. The dialog now appears once per version: on a first install, after an upgrade, and after deliberately installing an older build. Its close button waits a few seconds before it can be used, which is the only reason a dialog on startup can earn its place — without the pause it is dismissed on reflex before anything is read. Closing it is what marks it as seen, so a window reloaded before you got to it shows it again; after that it never returns for that version. A release with nothing worth interrupting for shows no dialog at all.
- **A "Badge Legend" section in the Help dialog**, listing every badge the graph can draw beside what it means: the three branch states, a branch checked out in a worktree, the HEAD marker, tags and stashes. The samples are real badges rendered in your own first graph-lane color rather than pictures of them, so the legend cannot quietly stop matching what the graph draws. The same section appears in this release's "What's new" dialog.
...
### Fixed
- **Rebase, cherry-pick, revert and Drop Commit no longer refuse work that Git itself accepts.** These actions were gated behind a check that called the working tree "dirty" if it contained anything at all — including untracked files Git does not care about. A repository whose only change was an untracked folder could not be rebased, with the message "Working tree has uncommitted changes", even though `git rebase` runs happily in that state. Cherry-pick and revert were stricter still: Git allows both when your local edits touch different files than the patch does, but any local change at all blocked them here. Opening the interactive rebase dialog was blocked too, though listing commits only reads history. Each action now runs and lets Git apply its own rule, and when Git does refuse, its message is shown as-is — naming the exact files in the way, instead of a generic sentence. Removing a worktree still asks for **force** when it contains modified or untracked files, because that is Git's own rule there; its message now says so.
- **Reverting with a custom message no longer sweeps your staged work into the revert commit.** That mode stages the reverse patch and then commits, so anything you had already staged went in with it, under the revert's message. Git refuses every other commit-producing revert while the index is dirty; this one now does the same and says why.
- **Starting a rebase while another rebase, cherry-pick, revert or merge is unfinished is refused up front**, instead of failing deep inside Git and being reported as a fresh conflict. An interactive rebase in that state used to take over the paused rebase's scratch files, which broke its reword and squash messages on **Continue**.
- **Diffs for uncommitted and staged files now compare against the real HEAD.** The left-hand side was taken from the newest row of the graph, which is only HEAD by coincidence — on a detached checkout, or after a reset with a stash left behind, it could be a different commit entirely.
...
### Fixed
- **"Drop Commit" no longer disappears while a filter is active.** Whether a commit could be dropped was judged against the commits currently *on screen*, so switching on an author or search filter broke the chain between the branch tip and the commit and quietly removed the item from every menu — including commits that were perfectly droppable. The decision is now made against the repository's history rather than the filtered view, so filtering changes what you see and never what you can do.
- **"Drop Commit" is no longer offered where dropping would flatten a merge.** A commit merged in from a side branch was treated as droppable, but dropping it replays the history above it and silently discards any merge commit along the way — the user asked to remove one commit and got a linearised branch. Drop is now offered only where the branch's history is linear from its tip down to the commit.
- **Branches whose names contain a slash are no longer mistaken for remote branches.** A local branch such as `eric/wip` or `jira/PROJ-123` was classified by guessing whether the part before the slash was a remote name, using a hardcoded list of prefixes (`feature`, `bugfix`, `test`, …). Any prefix outside that list turned a local branch into a phantom remote branch — and when it was the branch you had checked out, the extension no longer recognised it as current, so its badge menu offered Checkout, Merge into Current Branch, and Delete Branch for the branch you were already on. Branch names are now read from Git in fully-qualified form (`refs/heads/…` / `refs/remotes/…`), so local branches, remote branches, and remotes with any name are told apart exactly rather than by pattern. The same applies to commit decorations, so a remote that isn't called `origin`, `upstream`, or `fork` now shows its branches as remote branches instead of local ones, and to the "contained in" branch list in the commit tooltip, which could not tell a local `eric/wip` from `origin/eric/wip` and dropped any local branch whose name ended in `/HEAD`.

- **"Go to HEAD" is no longer cancelled by an unrelated background error.** Any failing request — a commit's details, a signature check — quietly ended a navigation that was still loading commits, so the button stopped spinning, the graph never moved, and the only message shown was about something else entirely. Failures are now matched to the navigation that caused them.

### Changed
- **"Go to HEAD" is now instant when the HEAD commit is already on screen.** Every click walked the entire history to compute HEAD's exact position, even when the row was a few lines away — on a large repository that meant a multi-second pause for a jump the extension could already make. It now confirms the on-screen HEAD with a single cheap lookup and scrolls straight to it; the full search still runs, as before, only when HEAD is genuinely outside the loaded commits.
- **`speedyGit.batchCommitSize` is now capped at 10000.** The setting had no upper bound, so a very large value made a single load ask Git for an unbounded number of commits — and, above 10000, made "Go to HEAD" fetch *smaller* batches than configured. Values above the cap are clamped.
- **Context menus are grouped by what each action acts on, and the groups are now named.** Menu items are organised into groups for the object they act on — the ref itself, the commit under it, the remote, the worktree, compare — and each group's name is printed on its separator line as a small caption. A badge menu opens with the ref it was opened from named in full (**Branch  my-feature**, **Tag  v1.2.0**), and the commit row menu names the commit, so a menu opened over a dense graph always says what it will act on. Previously a branch menu put checkout, merge, rename, push and fast-forward in one undifferentiated block, and the commit menu spread fifteen items across eight unnamed groups. The captions sit on the existing separator lines, so no menu got taller. Menus small enough to read at a glance (stash, author, date) are unchanged.
- **Clipboard actions moved into a single "Copy" submenu.** Copy Commit Hash, Copy Short Hash and Copy Commit Message were three rows at the bottom of every commit menu; they are now one row that opens on hover. Branch and tag badge menus fold Copy Branch Name / Copy Tag Name into the same submenu, so everything copyable from a row lives behind one entry.
- **Submenu items now show a chevron** and stay highlighted while their submenu is open. Nothing previously distinguished "Reset Current Branch to Here" — which opens a submenu — from an item that acts on click.
- **Long context menus now scroll instead of running off the screen.** A branch badge's menu can reach around twenty items, which did not fit in a short editor window — the bottom of the menu was simply unreachable. Menus are now capped to the space actually available between where you clicked and the window edge, and scroll with the wheel or the arrow keys, using a scrollbar styled to match the editor's own. Menus also keep a small gap from the window edge instead of sitting flush against it.
- **Menu rows are slightly more compact** (28px instead of 32px), closer to VS Code's own menus, so fewer menus need to scroll at all.
...
- **"Drop Commit" and "Reset Current Branch to Here" are now styled as destructive actions too**, in the same red as Delete Branch. Inside the reset submenu only **Hard (discard all)** is coloured — soft and mixed keep your work in the working tree, so marking them as destructive would have diluted the warning where it matters.
...
### Added
- **"Go to HEAD" toolbar button.** A new **HEAD** button (next to Fetch, with its own separator) jumps the graph straight to the commit you have checked out — no more hunting for the small HEAD indicator when your checkout is far down the history. Clicking it selects the HEAD row, scrolls it to the center of the view, and briefly flashes the row so it's easy to spot. If the commit details panel is already open it switches to the HEAD commit; navigation never opens or closes the panel on its own. It works even when HEAD is thousands of commits deep and not loaded yet: the extension locates HEAD's exact position in the history and loads commits up to it in large targeted batches instead of paging through one batch at a time. Detached HEAD checkouts are supported, and clear messages explain the cases where HEAD can't be shown — when it's excluded by the current branch or date filter, hidden by an author or search filter, or the repository has no commits yet. The button shows a busy state while locating/loading and is safe to use alongside refreshes, repo switches, and background loading.
