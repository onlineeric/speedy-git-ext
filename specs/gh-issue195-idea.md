...
- Conflicting Git operations initiated from different tabs: no new extension-level lock. Git's own protections (`index.lock`, in-progress rebase/merge/cherry-pick via the existing `OperationGuard`) decide, and failures surface as they do today. Tabs showing the same working tree show a **notice** that another view is running an operation; their controls are **not** disabled. Disabling would be a lock by another name, and the one this feature deliberately does not add — if two views really collide, git reports the error.
- A dialog may become stale when another tab or external tool changes the repository. Revalidation is limited to actions whose meaning depends on where a ref points — reset, rebase onto, force-push, delete branch, drop commit. These are exactly the actions git will *not* refuse on its own: `reset --hard`, `push --force` and `branch -D` all succeed against a ref that moved. Immediately before running, re-read the target; if it moved, **do nothing** and explain "changed since you opened this dialog". The user reopens the dialog and acts on the refreshed state — there is no re-run-anyway path, because a payload built from the commits the user saw (an interactive rebase plan, above all) must never be replayed against a different tip. Other dialogs rely on Git failing.
...
Hidden graphs keep their state in memory (`retainContextWhenHidden` stays on), so switching back is instant. There is no tab cap. Before release, measure memory with about 5 tabs on large repositories. The measurement is judged on **shape**, not on an absolute figure: each additional graph must cost about what the previous one cost, and scrolling in the focused graph must feel the same with four hidden graphs as with none. The absolute number is recorded, not gated on; if it is uncomfortable, the answer is documentation pointing at `speedyGit.batchCommitSize`, not a tab cap or a per-tab batch reduction.
