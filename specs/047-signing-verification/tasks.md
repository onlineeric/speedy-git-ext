...
- [X] T015 [P] [US2] Create `webview-ui/src/components/SignatureColumnCell.tsx`: a memoized cell reading both `signaturePresence[hash]` and `signatureCache[hash]` by hash (O(1)). Render decision: presence `not-signed` → blank cell (terminal, FR-007); a cached `CommitSignatureInfo` → its grouped glyph via `signatureGlyph.ts`; otherwise (presence unknown, or `signed` with verdict not yet cached) → render nothing while the async passes resolve (no spinner per row) (depends on T010, T013).
